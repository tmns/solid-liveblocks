...
import type { Context, JSX, Resource } from 'solid-js'
...
> = {
  ...
  suspense: {
    ...
    /**
     * Returns a Solid resource, that when ready provides access to an object that lets you get
     * information about all the users currently connected in the room.
     *
     * @example
     * const others = useOthers();
     *
     ...
     * )
     */
    useOthers(): Resource<Others<TPresence, TUserMeta>>
     ...
     * those cases, you'll probably want to use a `shallow` comparison check.
     *
     * Note, this Suspense version returns a Solid resource.
     *
     * @example
     * const avatars = useOthers(users => users.map(u => u.info.avatar), shallow);
     * const cursors = useOthers(users => users.map(u => u.presence.cursor), shallow);
    ...
    /**
     * Returns a Solid resource that when ready provides access to an array of connection IDs.
     * This matches the values you'll get by using the `useOthers()` hook.
     *
     * Roughly equivalent to:
     *   useOthers((others) => others.map(other => other.connectionId), shallow)
     *
     ...
     * // Resource<[2, 4, 7]>
     */
    useOthersConnectionIds(): Resource<readonly number[]>
     ...
     * item pairwise.
     *
     * Note, that this Suspense version returns a Solid resource.
     *
     * For example, to select multiple properties:
     *
     * @example
     ...
     * @example
     * // Returns only the selected values
     * const other = useOther(2, user => user.presence.cursor);
     * // Resource<{1, 2}>
     */
    useOther<T>(
      connectionId: number,
     ...
     * const { x, y } = me().presence.cursor;
     */
    useSelf(): Resource<User<TPresence, TUserMeta>>
